...
* **A380이 사용하는 게이트는 C1~C6, D1~D6뿐.** 이 12개 게이트만 표시하고, 그 외 게이트(C7 이상, D7 이상, A/B/E 등 다른 계열 포함)가 명시된 항목은 모두 제외한다.
* 구현: 게이트 키워드 뒤의 게이트 토큰을 통째로 잡은 뒤 `^[CD][1-6]$`에 맞는 것만 통과시킨다. 따라서 두 자리(C10, D12)·다른 계열(A3, B12)·순수 숫자(312)는 모두 제외된다. (구버전 `[CD]\d`는 한 자리 C/D만 매칭해 C10·A3 등이 필터를 빠져나갔음 → 수정됨)
* §2에 따라 GATE / STAND / SPOT / BAY / REMOTE 표현 모두 동일하게 적용한다.
...
| #   | 규칙                                                                    | 구현 위치                                               | <br> |
| :-- | :-------------------------------------------------------------------- | :-------------------------------------------------- | :----------------------------- |
| 7-1 | NOTAM PACKAGE 2 전체 블록을 먼저 추출                                          | `extractNotamPackage2()`                            | <br> |
| 7-2 | `[ETP] ICAO / IATA / 공항 전체 이름` 형식의 제목줄로 섹션을 분리                        | \`etpRe = /(\[ETP]\[^\n]*)\n(\[\s\S]*?)(?=\[ETP]    | $)/gi\`                        |
| 7-3 | OFP 상단 "ETP :" 열에 나열된 공항과 동일한 공항이 `[ETP]` 섹션으로 등장                     | 동일 정규식으로 처리됨                                        | <br> |
| 7-4 | 각 ETP 공항 제목을 **✈️ \[ETP] ICAO / IATA / 공항명** 형태로 큰 글씨(cyan, bold)로 표시 | `buildEtpNotamDisplayLines()` — `type: 'etp-title'` | <br> |
| 7-5 | 제목 이하 NOTAM은 기존 `buildNotamDisplayLines()` 규칙(필터링 포함)을 그대로 적용         | `buildEtpNotamDisplayLines()` 내부에서 호출               | <br> |
| 7-6 | EDTO NOTAM 탭 헤더에 ETP 공항 코드 목록을 표시 (예: RJCC / PANC / PACD)             | `renderEnrteNotamPage()`                            | <br> |
